...
# We will be testing the ability to extend the unit system, so set that up
class ExtraUnits(object):
    def __getitem__(self, obs_type):
        if obs_type.endswith('Temp'):
            # Things that end with "Temp" is assumed to be in group_temperature
...
def setup():
    ...
    # get settings that we have to reset when done
    settings['tz'] = os.environ.get('TZ')
    settings['locale'] = locale.getlocale()

    # set timezone so we have consistent information for the tests
    ...
    # Find the configuration file. It's assumed to be in the same directory
    # as me, so first figure out where that is.
    ...
    weewx.xtypes.xtypes.append(weewx.wxxtypes.WXXTypes(altitude_vt,
                                                       latitude, longitude))

    # We test accumulator configurations as well, so initialize the Accumulator
    # dictionary:
    weewx.accum.initialize(config_dict)

    # The tests also test the examples in the 'example' subdirectory.
    ...
    # Monkey patch to create SLEs with unambiguous names. We will test using
    # these names.
    ...
    # add the units we will use in the tests
...
def teardown():
    ...
    # put things back to the way they were before we started
...
class Common(object):
    ...
    def setUp(self):
        ...
        # be sure no one else left things behind that will conflict
        root_dir = self.config_dict['WEEWX_ROOT']
        shutil.rmtree(root_dir, ignore_errors=True)
        # generate the test databases
    ...
    def test_report_engine(self):
        ...
        # The generation time should be the same as the last record in the test
        # database:
        testtime_ts = gen_fake_data.stop_ts
        print("test time is %s" % weeutil.weeutil.timestamp_to_string(testtime_ts))
    ...
    def run_engine(self, stn_info, record, testtime_ts):
        ...
        # Make SKIN_ROOT point to the correct location
        t.config_dict['StdReport']['SKIN_ROOT'] = os.path.join(DIR, 'test_skins')

        # Although the report engine inherits from Thread, we can just run it
        # in the main thread:
        ...
        # Walk the directory of expected results to discover all the generated
        # files we should be checking
        for dirpath, _, dirfilenames in os.walk(expected_dir):
            for dirfilename in dirfilenames:
                ...
                # Get the path relative to the directory of expected results
                    ...
                    with open(expected_filename_abs, 'r') as expected:
                        ...
                        while True:
                            ...
                            if actual_line == '' and expected_line == '':
                                break
                            n += 1
                            msg = "%s[%d]" % (actual_filename_abs, n)
                            self.assertEqual(actual_line, expected_line, msg=msg)
