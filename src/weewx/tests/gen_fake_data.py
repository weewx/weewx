...
def provision_binding(config_dict, binding, synthetic_dict):
    """Provision a particular binding.

    Args:
        config_dict (dict): A WeeWX configuration dictionary.
        binding (str): The binding to provision.
        synthetic_dict (dict): The dictionary containing the synthetic data parameters.
    """

    def make_nulls_filter(record):
        """Values of inTemp for the first half of the database will have nulls,
        the second half will not."""
        if record['dateTime'] > (synthetic_dict['start_ts'] + synthetic_dict['stop_ts']) / 2:
            record['inTemp'] = 68.0
        return record

    # Check to see if the data for the given binding already exists and is configured correctly.
    try:
        with weewx.manager.open_manager_with_config(config_dict, binding) as manager:
            if manager.firstGoodStamp() == synthetic_dict['start_ts'] \
                    and manager.lastGoodStamp() == synthetic_dict['stop_ts']:
                # Before weewx V4, the test database had interval in seconds.
                # Check that this one has been corrected.
                last_record = manager.getRecord(synthetic_dict['stop_ts'])
                if last_record['interval'] == synthetic_dict['interval'] / 60:
    ...
    # creating the daily summaries on the fly. First, we need to modify the
    # configuration dictionary that was passed in so it uses the regular Manager class,
    # instead of the DailySummaryManager.
    ...
    with weewx.manager.open_manager_with_config(monkey_dict, binding, initialize=True) as archive:
        ...
        log.info(
            "Creating synthetic database %s" % config_dict['DataBindings'][binding]['database'])
    ...
    # Now backfill using the original configuration file
    with weewx.manager.open_manager_with_config(config_dict, binding, initialize=True) as archive:
        ...
        patch_database(config_dict, binding,
                       synthetic_dict['start_ts'], synthetic_dict['stop_ts'])
        tdiff = time.time() - t1
        print("\nTime to patch database with derived types: %.2f seconds" % tdiff)


def patch_database(config_dict, binding, start_ts, stop_ts):
...
def gen_fake_records(start_ts, stop_ts, interval=1800,
                     amplitude=1.0,
                     day_phase_offset=math.pi / 4.0,
    ...
    for ts in range(start_ts, stop_ts + interval, interval):
        ...
        if math.sin(weather_phase) > .95:
            record['rain'] = 0.08 * amplitude if math.sin(
                weather_phase) > 0.98 else 0.04 * amplitude
        else:
            record['rain'] = 0.0
        ...
        for obs_type in ['barometer', 'outTemp', 'windDir', 'windGust', 'windGustDir',
                         'windSpeed']:
            count += 1
            if count % 71 == 0:
