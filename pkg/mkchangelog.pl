    ...
    while(<$IFH>) {
        ...
        if ($line =~ /^(\d+\.[0-9.X]+)/) {
            ...
            if ($version ne q()) {
                ...
                # if a release version was specified, check the first version
                # number in the changelog and ensure that it matches the release
                # version.
...
#                    exit 1;
            ...
            @paragraphs = (); # ignore anything before the first valid version
            $version = $v;
            $ts = time;
            if ($line =~ /(\d+)\/(\d+)\/(\d\d)/) {
