...
syslog_to_logging = {   LOG_EMERG   : logging.CRITICAL,
...
class syslogLogger:
    ...
    def openlog(self, ident=sys.argv[0], logoptions=0, facility=LOG_USER):
        ...
        # Local File Handler
        # first try creating log in 'LOCALAPPDATA\<indent>\logs' folder
        # if unsuccessful, create log as 'LOCALAPPDATA\<ident>.log'
        try:
            self.file_handler = handlers.RotatingFileHandler(os.path.join(os.environ.get('LOCALAPPDATA'), ident + '/logs/' + ident + '.log'), maxBytes = 10 * 1024 * 1024, backupCount = 10)
        except:
            self.file_handler = handlers.RotatingFileHandler(os.path.join(os.environ.get('LOCALAPPDATA'), ident + '.log'), maxBytes = 10 * 1024 * 1024, backupCount = 10)
        ...
        self.syslog_handler = handlers.SysLogHandler(address=('localhost',514), facility=facility)
        
        self.syslog_handler.setFormatter(syslog_formatter)

        self.logger.addHandler(self.syslog_handler)

        # Unix syslog defaults to all priority while loggin defaults to WARNING so set explicitly
