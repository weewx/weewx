...
class TestAggregate(unittest.TestCase):
    ...
    def get_aggregate_fn(self, aggregate_fn):
        with weewx.manager.open_manager_with_config(self.config_dict, 'wx_binding') as db_manager:
            ...
            avg_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'avg', db_manager)
            self.assertAlmostEqual(avg_vt[0], 28.74, 2)
            self.assertEqual(avg_vt[1], 'degree_F')
            self.assertEqual(avg_vt[2], 'group_temperature')

            max_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'max', db_manager)
            self.assertAlmostEqual(max_vt[0], 58.97, 2)
            maxtime_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'maxtime', db_manager)
            self.assertEqual(maxtime_vt[0], 1270087200)

            min_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'min', db_manager)
            ...
            if aggregate_fn == weewx.aggregate.get_aggregate:
                ...
                first_time_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'firsttime', db_manager)
                self.assertEqual(first_time_vt[0], ts)

                last_vt = aggregate_fn('outTemp', TimeSpan(start_ts, stop_ts), 'last', db_manager)
