...
def configure_driver(config_dict, cmd_driver=None, no_prompt=False, logger=None):
    """Do what's necessary to create or reconfigure a driver in the configuration file.

    Args:
        config_dict (configobj.ConfigObj): The configuration dictionary
        cmd_driver (str): The driver to use. Something like 'weewx.drivers.fousb'.
    ...
    # Did the user specify a driver on the command line?
    if cmd_driver:
        # Yes. Use it
        driver = cmd_driver
        # Fail hard if the driver cannot be imported
        importlib.import_module(driver)
    elif not no_prompt:
        # Prompt for a suitable driver
        driver = prompt_for_driver(driver)

    try:
        # Look up driver info, and get the driver editor. The editor provides a default stanza,
        # and allows prompt-based editing of the stanza.
        driver_editor, driver_name, driver_version = load_driver_editor(driver)
    except Exception as e:
        sys.exit("Driver %s failed to load: %s" % (driver, e))
    logger.log(f'Using {driver_name} version {driver_version} ({driver})')
    ...
    if stanza and driver_name:
        ...
        if 'Station' in config_dict:
            ...
            # make the stanza the station type
            config_dict['Station']['station_type'] = driver_name
        # Finally, give the user a chance to modify the stanza:
        if not no_prompt:
            settings = prompt_for_driver_settings(driver, config_dict)
